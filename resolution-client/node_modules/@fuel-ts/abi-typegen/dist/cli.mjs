...
*/
`;function x(s){let{data:t,template:e}=s,r={strict:!0,noEscape:!0},n=xt(e,r),o=xt(At,r);return n({...t,header:o(zt)}).replace(/[\n]{3,}/gm,`
...
export type Vec<T> = T[];
`;function D(){return x({template:Et})}var wt=`{{header}}
...
`;function Gt(s){let{abi:t}=s,{types:e}=t,{rawContents:r,name:n,hexlifiedBinContents:o}=s.abi,i=JSON.stringify(r,null,2),a=t.functions.find(b=>b.name==="main");if(!a)throw new Error("ABI doesn't have a 'main()' method.");let{enums:u}=V({types:e}),{structs:y}=B({types:e}),{imports:l}=z({types:e,baseMembers:["Script","Account"]}),{prefixedInputs:T,output:I}=a.attributes;return x({template:Lt,data:{inputs:T,output:I,structs:y,enums:u,abiJsonString:i,hexlifiedBinString:o,capitalizedName:n,imports:l}})}function Pt(s){let{abis:t,outputDir:e}=s,r=[],n=t.find(i=>i.commonTypesInUse.length>0);t.forEach(i=>{let{name:a}=i,y={path:`${e}/factories/${a}__factory.ts`,contents:Gt({abi:i})};r.push(y)});let o={path:`${e}/index.ts`,contents:v({abis:t})};if(r.push(o),n){let a={path:te(e,"common.d.ts"),contents:D()};r.push(a)}return r}import ee from"lodash.upperfirst";function tt(s){let{abiFilepath:t,binFilepath:e,binExists:r,programType:n}=s,o=n==="script";if(!r&&o)throw new Error([`Could not find BIN file for counterpart ${ee(n)} ABI.`,`  - ABI: ${t}`,`  - BIN: ${e}`,n].join(`
`))}var et=class{abis;abiFiles;binFiles;outputDir;files;constructor(t){let{abiFiles:e,binFiles:r,outputDir:n,programType:o}=t;this.outputDir=n,this.abiFiles=e,this.binFiles=r,this.abis=this.abiFiles.map(i=>{let a=i.path.replace("-abi.json",".bin"),u=this.binFiles.find(({path:l})=>l===a);return u||tt({abiFilepath:i.path,binExists:!!u,binFilepath:a,programType:o}),new W({filepath:i.path,rawContents:JSON.parse(i.contents),hexlifiedBinContents:u==null?void 0:u.contents,outputDir:n,programType:o})}),this.files=this.getAssembledFiles({programType:o})}getAssembledFiles(t){let{abis:e,outputDir:r}=this,{programType:n}=t;switch(n){case"contract":return St({abis:e,outputDir:r});case"script":return Pt({abis:e,outputDir:r});case"predicate":return Ot({abis:e,outputDir:r});default:throw new Error(`Invalid Typegen programType: ${n}`)}}};function Ht(s){let{cwd:t,inputs:e,output:r,silent:n,programType:o,filepaths:i}=s,a=pe(t),{log:u}=console;n&&(u=()=>({}));let y=[];if(!(i!=null&&i.length)&&(e==null?void 0:e.length))y=e.flatMap(b=>se(b,{cwd:t}));else if(i!=null&&i.length)y=i;else throw new Error("You need to inform at least one parameter: `input` or `filepaths`");let l=y.map(b=>({path:b,contents:Mt(b,"utf-8")})),I=o==="script"?y.map(b=>{let E=b.replace("-abi.json",".bin"),Xt=ne(E);return tt({abiFilepath:b,binFilepath:E,binExists:Xt,programType:o}),{path:E,contents:re(Mt(E))}}):[],f=new et({outputDir:r,abiFiles:l,binFiles:I,programType:o});u(`Generating files..
`),oe.sync(`${r}/factories`),f.files.forEach(b=>{ae.sync(b.path),ie(b.path,b.contents);let E=new RegExp(`^.+${a}/`,"m");u(` - ${b.path.replace(E,"")}`)}),u(`
Done.\u26A1`)}function ue(s){let{contract:t,script:e,predicate:r}=s;return t||!t&&!e&&!r?"contract":r?"predicate":"script"}function ye(s){let{inputs:t,output:e,silent:r,contract:n,script:o,predicate:i}=s,a=process.cwd(),u=ue({contract:n,script:o,predicate:i});Ht({cwd:a,inputs:t,output:e,programType:u,silent:!!r})}function le(s){s.requiredOption("-i, --inputs <path|glob...>","input paths/globals to your abi json files").requiredOption("-o, --output <dir>","directory path for generated files").addOption(new lt("-c, --contract","generate types for Contracts [default]").conflicts(["script","predicate"]).implies({script:void 0,predicate:void 0})).addOption(new lt("-s, --script","generate types for Scripts").conflicts(["contract","predicate"]).implies({contract:void 0,predicate:void 0})).addOption(new lt("-p, --predicate","generate types for Predicates").conflicts(["contract","script"]).implies({contract:void 0,script:void 0})).option("-S, --silent","omit output messages").action(ye)}function Ni(s){let t=new me,{argv:e,programName:r}=s;t.name(r),t.version(ce.FUELS),t.usage("-i ../out/*-abi.json -o ./generated/"),le(t),t.parse(e)}export{le as configureCliOptions,ue as resolveProgramType,Ni as run,ye as runCliAction};
